...
extension MasterDataService {
    func listUnitStories() throws -> [UnitStoryGroup] {
        var storyRecords = try dbQueue.read { db in
            ...
            for row in try Row.fetchAll(try db.makeStatement(literal: """
        ...
        let names = try stringService.getGroupTitles(forEntities: storyRecords.map { $0.id }, inDomain: "unit")
        storyRecords.applyGroupTitles(names)
        let allScripts = storyRecords.flatMap { $0.episodes.map { $0.script } }
